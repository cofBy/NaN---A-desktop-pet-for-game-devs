...
public class Robot : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        hit = new RaycastHit2D[Targets.Length];
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < Targets.Length; i++)
        {
            ...
            hit[i] = Physics2D.Raycast(FootPlaces[i].position + new Vector3(0, AdditonalY, 0), Vector3.down, int.MaxValue, WalkableMask);
            if (hit[i].collider.gameObject != null)
            {
